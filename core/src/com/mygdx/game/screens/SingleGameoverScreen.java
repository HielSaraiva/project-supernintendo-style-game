...
public class SingleGameoverScreen implements Screen {
    ...
    public SingleGameoverScreen(SpaceInvaders game, int score) {
        this.game = game;
        this.score = score;
        ...
        greenFont = generator.generateFont(parameter); // or something like that
    ...
    public void render(float delta) {
        ...
        bitmap.draw(game.batch, "Best Score: " + SingleInvadersScreen.getHighscore(), ((float) Gdx.graphics.getWidth() - 800) / 2, (float) Gdx.graphics.getHeight() / 2 + 100);
        ...
        if (Gdx.input.getX() < (Gdx.graphics.getWidth() + menuButtonInactive.getWidth()) / 2 && Gdx.input.getX() > (Gdx.graphics.getWidth() - menuButtonInactive.getWidth()) / 2 &&
            ...
            game.batch.draw(menuButtonInactive, (float) (Gdx.graphics.getWidth() - menuButtonActive.getWidth()) / 2, (float) (Gdx.graphics.getHeight() - menuButtonActive.getHeight()) / 2 - 350);
        }
        game.batch.end();
    }
